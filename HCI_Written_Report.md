...
### 5.2 Technical Challenges and Solutions
*   **Challenge 1: Icon package compilation error.** During production builds, Vite reported that `Twitter` was not exported by `lucide-react`. 
    *   *Solution:* Inspected the installed package exports using a Node.js script. Discovered that the version installed uses the modern `X` icon export. Replaced imports and references, restoring successful production compilation.
*   **Challenge 2: Multi-Column layout wrapping.** On medium width viewports (tablets), the social profile links in the footer wrapped awkwardly, moving the "Email" link onto its own line.
    *   *Solution:* Redefined the footer's layout columns. Instead of equal grid divisions, used `1.2fr 0.8fr 1.5fr` for wider monitors to allocate more space to the links list. Reduced flex gaps to `0.75rem`. This aligns all links on a single line, preserving clean aesthetics.
*   **Challenge 3: Incorrect certificate verification links.** Some SoloLearn and HackerRank certifications had duplicated verification links or wrong ID placeholders.
    *   *Solution:* Analyzed the local certificate screenshots. Cross-referenced properties with the profiles to locate the correct IDs (e.g. SoloLearn Python: `CC-ANH74SBO`, HackerRank SQL: `259d46322eb5`) and updated the data array.
